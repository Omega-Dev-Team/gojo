...
fn test_nonce_utils_functionality() {
    ...
    let (_, _, _, _, _, market_factory, _, data_store, _, _, _, _, _, _, _, _, _, _, _, _,) =
        setup();
    ...
    assert(key1 != key2, 'Diff nonces == diff keys');
    ...
    assert(final_nonce == 3, 'Nonce == 3 after get_next_key');
