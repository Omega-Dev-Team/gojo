...
use satoru::withdrawal::{
    ...
    withdrawal_utils::{
        create_withdrawal, cancel_withdrawal, swap, execute_withdrawal, CreateWithdrawalParams,
...
fn deploy_contracts() -> (
    ...
    let role_store_address = role_store_class.deploy(@array![caller.into()]).unwrap();
    ...
    let market_token_address = market_token
        .deploy(@array![data_store_address.into(), role_store_address.into()])
        .unwrap();

    let oracle_store = declare('OracleStore');
    let oracle_store_address = oracle_store
        .deploy(@array![role_store_address.into(), event_emitter_address.into()])
        .unwrap();
    ...
    let oracle_address = oracle
        .deploy(
            @array![
                ...
                contract_address_const::<0x123>().into()
            ]
        )
        .unwrap();
...
fn test_swap() {
    ...
    let params = ExecuteWithdrawalParams {
        ...
        withdrawal_vault: IWithdrawalVaultDispatcher {
            contract_address: withdrawal_vault.contract_address
        },
